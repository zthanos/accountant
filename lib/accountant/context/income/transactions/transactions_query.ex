defmodule Accountant.Context.Transactions.TransactionsQuery do
  ...
  def paginate(params) do
    ...
    # res |> dbg()
    totals =
      Repo.all(from p in query,
      group_by: p.sales_category,
      select: %{ sales_category: p.sales_category, net_total: sum(p.net_amount),  vat_total: sum(p.vat_amount), gross_total: sum(p.gross_amount)}
      )
  ...
  defp aggregate_sales(query) do
      ...
      query =
        query
        |> group_by([t], t.sales_category) # Modify the grouping criteria as needed

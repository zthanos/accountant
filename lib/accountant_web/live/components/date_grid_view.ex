defmodule AccountantWeb.Components.DateGridView do
  defstruct [:selected_date,  :view_date, :month_name, :week_names, :current_month, :current_year, :popover_id, :anchor_id, :dates]
  ...
  defp get_month_grid!(date, selected_date) do
    ...
    prefix = case first_day_of_week do
    ...
    dates = Timex.Interval.new(from: min_date, until: max_date)
    |> Timex.Interval.with_step([days: 1])
    |> Enum.map(fn x -> %{day: x, selected: Timex.compare(x, selected_date) == 0, current_month: x.month == date.month} end)
